...
;
; Rolling desktops forward
; Automatically show first desktop when going past the last.
;
rollDesktopsNext() {
    global CurrentDesktop, DesktopCount
    mapDesktopsFromRegistry()
...
;
; Rolling desktops backward
; Automatically show last desktop when going past the first.
;
rollDesktopsPrevious() {
    global CurrentDesktop, DesktopCount
    if --CurrentDesktop < 1
        CurrentDesktop := DesktopCount
    switchDesktopByNumber(CurrentDesktop)
...
;^#k::rollDesktopsPrevious()
;^#l::rollDesktopsNext()
;^#1::switchDesktopByNumber(1) ; LWin & 1::switchDesktopByNumber(1)
